...
INFLATION_FIXTURE = ROOT / "data" / "fixtures" / "cpih_sample.csv"
...
def run(live: bool = False) -> Path:
    ...
    inflation = pd.read_csv(INFLATION_FIXTURE, parse_dates=["month"])
    DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    with duckdb.connect(str(DB_PATH)) as connection:
        connection.register("awards_input", awards)
        connection.register("inflation_input", inflation)
